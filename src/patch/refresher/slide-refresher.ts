...
import { Content } from '../../../node_modules/ionic-angular/components/refresher/../content/content';
import { GESTURE_PRIORITY_REFRESHER, GESTURE_REFRESHER, GestureController, GestureDelegate } from '../../../node_modules/ionic-angular/components/refresher/../../gestures/gesture-controller';
import { isTrueProperty } from '../../../node_modules/ionic-angular/components/refresher/../../util/util';
import { Platform } from '../../../node_modules/ionic-angular/components/refresher/../../platform/platform';
import { pointerCoord } from '../../../node_modules/ionic-angular/components/refresher/../../util/dom';
import { PointerEvents } from '../../../node_modules/ionic-angular/components/refresher/../../gestures/pointer-events';
import { UIEventManager } from '../../../node_modules/ionic-angular/components/refresher/../../gestures/ui-event-manager';

//xyz
import { ElementRef } from '@angular/core';
import { Slide } from '../../../node_modules/ionic-angular/components/refresher/../slides/slide';
 ...
 */
@Directive({
  selector: 'slide-refresher',
  host: {
    '[class.refresher-active]': 'state !== "inactive"',
    '[style.top]': '_top'
  }
})
export class SlideRefresher {
   ...
   */
  @Output() ionRefresh: EventEmitter<SlideRefresher> = new EventEmitter<SlideRefresher>();

  /**
   * @output {event} Emitted while the user is pulling down the content and exposing the refresher.
   */
  @Output() ionPull: EventEmitter<SlideRefresher> = new EventEmitter<SlideRefresher>();

  /**
   * @output {event} Emitted when the user begins to start pulling down.
   */
  @Output() ionStart: EventEmitter<SlideRefresher> = new EventEmitter<SlideRefresher>();

  //xyz add eleref
  constructor(private _plt: Platform, private _content: Content, private _zone: NgZone, gestureCtrl: GestureController,private eleref:ElementRef) {
  ...
  _onStart(ev: TouchEvent): any {
    ...
    if (!xyzrm && this._content.contentTop > 0) {
  ...
  _onMoveInZone() {
    ...
    if (!xyzrm && this.deltaY > this.pullMax) {
  ...
  _setCss(y: number, duration: string, overflowVisible: boolean, delay: string) {
    ...
    if (xyzadd) {
      ...
      if (this.state == STATE_PULLING) {
        ...
        el.style.top = Math.min(this.pullMax - this.pullMin, y) - 30 + 'px';
        ...
        //ion-icon
        // let pullicon = this.eleref.nativeElement.querySelector('ion-icon');
        let pullicon = this.eleref.nativeElement.querySelector('img');
        pullicon.style.transform = "rotate(" + ((y * 3) - 720) + "deg)";
      }
      else if (this.state == STATE_COMPLETING || this.state == STATE_CANCELLING || this.state==STATE_INACTIVE) {
        el.style.top = '';  
        el.style.transition = '';    
      }
      return;
