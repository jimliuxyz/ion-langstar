...
class ShortInfo{
  constructor(public views: number, public likes) { }
}

export class BookInfoService extends DataService{
  readonly data$: ReplaySubject<BookInfo> = new ReplaySubject();
  ...
  private async joleByTagList(isJoin: boolean, book: BookInfo, langpair: string, tagname: string) {
    ...
    const path = [...BYTAG, langpair, tagname, book.uid];

    if (isJoin) {
      const short_info = new ShortInfo(book.views, book.likes);
